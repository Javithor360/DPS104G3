...
const ProductItem = ({ producto, onAgregar }: ProductItemProps) => (
  <div
    style={{
      background: "#fff",
...
// ── Sub-componente: resumen del carrito ───────────────────────
const CartSummary = ({ carrito, onEliminar, onLimpiar }: CartSummaryProps) => {
  // .reduce() para calcular el total
  const total: number = carrito.reduce(
    (acc: number, item: ItemCarrito) => acc + item.precio * item.cantidad,
    0,
  );
  const totalItems: number = carrito.reduce(
  ...
  return (
    <div>
      ...
      >
        <h3 style={{ margin: 0, color: "#001f3f" }}>
          Tu carrito ({totalItems} items)
        </h3>
        <button
          onClick={onLimpiar}
          style={{
      ...
      >
        <div style={{ display: "flex", justifyContent: "space-between" }}>
          ...
          <span
            style={{ fontWeight: 800, fontSize: "1.15rem", color: "#003f7f" }}
          >
    ...
    >
      ...
      <div
        style={{
          ...
          borderRadius: "12px",
          padding: "20px",
          boxShadow: "0 2px 10px rgba(0,63,127,.08)",
        }}
      >
        <CartSummary
          carrito={carrito}
